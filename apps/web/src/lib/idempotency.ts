...
type Entry = { storedAt: number; body: unknown };
...
export function idemGet(listId: number, idemKey?: string): unknown | null {
...
export function idemSet(listId: number, idemKey: string | undefined, body: unknown) {
  if (!idemKey) return;
  const k = keyFor(listId, idemKey);
